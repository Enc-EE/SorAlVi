export class AlgorithmEngine {
    ...
    private _numberOfNumbers: number = 0;
    ...
    public executeAction = (action: AlgorithmAction) => {
        switch (action.t) {
            case AlgorithmActionType.trace:
                ...
                const existingHighlight = this._currentHighlights.find(x => x.k === k)
                if (existingHighlight) {
                    this._currentHighlights[this._currentHighlights.indexOf(existingHighlight)] = action as AlgorithmTraceAction
                } else {
